'''This file makes a request to themoviedb.org's api and fetches the following movie details:
title, storyline/overview, poster image url, youtube trailer url and the release date
'''
import media
...
MOVIE_LIST = requests.get("https://api.themoviedb.org/3/movie/upcoming?api_key=" +
                          "03531cf9267cbb1ee25c65670346beca&page=1")
...
for item in MOVIE_DATA["results"]:
    # Loop through the response received from api.themoviedb.org and pick out the
    # title, overview and movie image
    movie_title = item["title"]
    movie_storyline = item["overview"][0:200].encode('utf8') + "...read more"
    poster_image = POSTER_BASE_URL + item["poster_path"]
    release_date = item["release_date"]

    # Get the video feed for each movie item and assign the video URL to trailer_youtube
    movie_id = str(item["id"])
    movie_trailer_list = requests.get(MOVIE_TRAILER_BASE_URL + movie_id + MOVIE_TRAILER_END_URL)
    movie_trailer_data = movie_trailer_list.json()
    trailer_youtube = YOUTUBE_TRAILER_BASE_URL + movie_trailer_data["results"][0]["key"]
